...
class TestFastNode:
    ...
    def test_fast_passes_chat_messages(self):
        ...
        assert call_kwargs.kwargs.get("messages") == messages or (call_kwargs.args and messages in call_kwargs.args)
...
class TestOrchestratorNode:
    ...
    def test_fast_path_short_task(self):
        ...
        assert "fast-path" in " ".join(result["history"]).lower() or "forced" in " ".join(result["history"]).lower()
    ...
    def test_llm_route_used_for_long_task(self):
        ...
        with (
            patch("nodes.orchestrator._call", return_value='{"route": "RESEARCHER", "done": false}') as mock_call,
    ...
    def test_qwen3_think_tags_stripped(self):
        ...
        with (
            patch("nodes.orchestrator._call", return_value=raw_with_think),
            patch("nodes.orchestrator._relevant_memory", return_value=""),
        ):
            result = orchestrator(state)
    ...
    def test_iterations_incremented(self):
        ...
        state = make_state(task=long_task, iterations=0)
        with (
            patch("nodes.orchestrator._call", return_value='{"route": "FAST", "done": false}'),
            patch("nodes.orchestrator._relevant_memory", return_value=""),
        ):
            result = orchestrator(state)
        assert result["iterations"] == 1
...
class TestStreamCallback:
    ...
    def test_callback_receives_tokens(self):
        ...
        with (
            patch("helpers.llm.completion", return_value=iter([fake_chunk])),
